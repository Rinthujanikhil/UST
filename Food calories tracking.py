...
@app.route('/delete/<string:food>',methods=['GET'])
def delete_item(food):
    collections.delete_one({'food':food})
    return redirect('/home')

@app.route('/edit/<string:food>',methods=['GET','POST'])
def edit_item(food):
    if request.method == 'POST':
        date1=request.form['date']
        item=request.form['food']
        ...
        carbo=float(request.form['carbo'])
        calorie=4*carbo+4*protein+9*fat
        collections.update_one({'food':food},{'$set':{'food':food,'date':date,'fat':fat,'protein':protein,'carbo':carbo,'calorie':calorie}})
        return redirect('/home')

    items=collections.find({'food':food})
    return render_template('update.html',food=items)
